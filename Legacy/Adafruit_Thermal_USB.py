    ...
    def printBarcode(self, text, type):
        ...
        if self.firmwareVersion >= 264:
            ...
            if self.writeToStdout:
                ...
                self._USB.write((chr(n)).encode('utf-8', 'ignore'))
                for i in range(n):
                    self._USB.write(text[i].encode('utf-8', 'ignore'))
        else:
            # Older firmware: write string + NUL
            if self.writeToStdout:
                sys.stdout.write(text.encode('utf-8', 'ignore'))
            else:
                self._USB.write(text.encode('utf-8', 'ignore'))
        self.prevByte = '\n'
            ...
            for y in range(chunkHeight):
                for x in range(rowBytesClipped):
                    if self.writeToStdout:
                        ...
                        self._USB.write(bytes([bitmap[i]]))
                    i += 1
